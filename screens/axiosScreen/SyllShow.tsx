...
type SubShowScreenProp = StackNavigationProp<RootStackParamList, 'SyllShow'>;


export default function SyllShow({route}: {route: SubShowScreenProp}) {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        await axios.post(url, jsonData).then(res => {
          const resData = res.data.syllabus;
          setData(resData);
        });
        setLoad(false);
      } catch (error) {
    ...
    <View style={{flex: 1, backgroundColor: '#F5F5F5'}}>
        ...
        <View style={styles.outerContainer}>
          <View style={styles.subjectCard}>
            ...
            {/* Create Post Button */}
            {acc && (
            <TouchableOpacity style={styles.createButton} onPress={()=> navigation.navigate('UploadSyll', {subname:"test"})}>
              <Icon
                name="plus"
                size={18}
                color="#fff"
