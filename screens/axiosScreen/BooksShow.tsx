...
import {
  ...
  StyleSheet,
  ScrollView,
  TouchableOpacity, ToastAndroid, Vibration
} from 'react-native';
...
type SubShowScreenProp = StackNavigationProp<RootStackParamList, 'BooksShow'>;

export default function BooksShow({route}: {route: SubShowScreenProp}) {
  ...
  function addLike(subname: string, postdate: string) {
    const fetchData = async () => {
      ...
      try {
        const url = API_URL + '/booklike';
        axios.post(url, jsonData).then(res => {
          const resData = res.data;
          if (resData === 'success') {
            ToastAndroid.show('Your Like Added', ToastAndroid.SHORT);
          } else {
            ToastAndroid.show('Your Already Liked', ToastAndroid.SHORT);
            Vibration.vibrate(100);
          }
        });
      } catch (error) {
  ...
  async function sendLocalNotification() {
    ...
    await notifee.displayNotification({
      ...
      body: 'Your Book PDF Deleted Successfully',
      android: {
        channelId: 'default',
              ...
              <TouchableOpacity
                ...
                <View style={{flexDirection: 'row', alignItems: 'center'}}>
                  ...
                  <Icon
                    ...
                    size={28}
                    color={
                      expandedSubject === outerIndex ? '#3F51B5' : '#757575'
                    }
                  />
                  {acc && (
                    <TouchableOpacity
                      style={{marginLeft: 15}}
                      onPress={() =>
                        navigation.navigate('UploadBooks', {subname: i.subname})
                      }>
